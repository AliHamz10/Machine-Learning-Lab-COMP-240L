...
class HeartDiseaseMLFramework:
    ...
    def load_data(self):
        ...
        try:
            ...
            try:
                ...
                # Check if target column exists in alternative source
                if "target" not in self.df.columns:
                    raise ValueError("Target column not found in alternative dataset")
                ...
                try:
                    ...
                    self.df = pd.read_csv(
                        data_path, names=self.column_names, na_values="?"
                    )
                    print("✓ Dataset loaded from local file")
                except:
                    print(
                        "❌ Could not load dataset. Please ensure internet connection or local file exists."
                    )
                    print(
                        "   Download from: https://archive.ics.uci.edu/ml/machine-learning-databases/heart-disease/"
                    )
                    return False
    ...
    def explore_data(self):
        ...
        print(
            f"Class 1 (Disease): {target_dist[1]} ({target_dist[1] / len(self.df) * 100:.2f}%)"
        )
        ...
        if imbalance_ratio < 0.5:
            print(
                "⚠ Warning: Significant class imbalance detected. Will use appropriate techniques."
            )
    ...
    def create_exploratory_plots(self):
        ...
        target_corr = (
            corr_matrix["target"].drop("target").abs().sort_values(ascending=True)
        )
        ...
        plt.savefig(
            os.path.join(self.results_dir, "heart_disease_eda.png"),
            dpi=300,
            bbox_inches="tight",
        )
        print(
    ...
    def preprocess_data(self):
        ...
        # 4. Handle class imbalance using SMOTE (before scaling to ensure consistency)
        print("\n4. Handling Class Imbalance:")
        imbalance_ratio = min(self.y_train.value_counts()) / max(
        ...
        if imbalance_ratio < 0.6:
            ...
            print(f"   ✓ Training data after SMOTE: {self.X_train.shape[0]} samples")
            print(
        ...
        # 5. Feature scaling (after SMOTE to ensure scaled data matches resampled data)
        print("\n5. Feature Scaling:")
    ...
    def hyperparameter_tuning(self):
        ...
        rf_grid = GridSearchCV(
            rf_base, rf_param_grid, cv=5, scoring="accuracy", n_jobs=-1, verbose=1
        )
        ...
        svm_grid = GridSearchCV(
            svm_base, svm_param_grid, cv=5, scoring="accuracy", n_jobs=-1, verbose=1
        )
    ...
    def plot_confusion_matrices(self):
        ...
        plt.suptitle(
            "Confusion Matrices - All Models", fontsize=16, fontweight="bold", y=0.995
        )
        plt.tight_layout()
        plt.savefig(
        ...
        for idx, name in enumerate(tree_models):
            if name in self.models:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    ax.set_title(
                        f"{name} - Feature Importance", fontsize=12, fontweight="bold"
                    )
                    ax.invert_yaxis()
                    ax.grid(axis="x", alpha=0.3)
    ...
    def plot_learning_curves(self):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.results_dir, "learning_curve.png"),
            dpi=300,
            bbox_inches="tight",
        )
    ...
    def plot_radar_chart(self):
        ...
        for idx, row in self.comparison_df.iterrows():
            ...
            ax.plot(
                angles, values, "o-", linewidth=2, label=row["Model"], color=colors[idx]
            )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.results_dir, "radar_chart.png"),
            dpi=300,
            bbox_inches="tight",
        )
